...
FixStyle(smoothforce,FixSmoothForce)

#else

#ifndef LMP_FIX_SMOOTHFORCE_H
#define LMP_FIX_SMOOTHFORCE_H

#include "fix.h"

namespace LAMMPS_NS {

class FixSmoothForce : public Fix {
 public:
  FixSmoothForce(class LAMMPS *, int, char **);
  ~FixSmoothForce();
  int setmask();
  void init();
  void setup(int);
 ...
 private:
  double xvalue,yvalue,zvalue,xlo,xhi,delta,omega,fmax;
  int varflag,iregion;
